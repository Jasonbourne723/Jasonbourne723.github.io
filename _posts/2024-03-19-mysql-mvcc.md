...
> 这里注意一下：MVCC解决的只是 RR 级别下快照读的幻读问题，而当前读的幻读问题则是通过间隙锁来解决的。也就是说 RR 级别下是通过 MVCC+间隙锁 来解决大部分幻读问题的。
